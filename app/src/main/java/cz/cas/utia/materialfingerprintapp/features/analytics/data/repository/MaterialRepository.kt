...
// a polar plot screen bude taky mit ten stejny dialog (udelat nekde jako composablu znovupouzitelnou), zda chce uzivatel vyhledat podobny material mezi remote nebo local materialy
// a kdyz uzivatel bude hledat podobny material k remote materialu na polar plot screene, tak do apply filter screen slotu ulozit tech 16 statistik a zavolat navigaci na browse similar remote materials a prametrem -1L (a pro local se preda normalne idcko) - vubec nevadi, ze to prepise apply filter slot, protoze ten se reloaduje pouze kdyz se jde aplikovat filter na nejaky polar plot - viz nize

//todo apply filter screen:
// kdyz tam jdu z home analytics screen, tak nechci loadovat charakteristiky z proto data store, ale kdyz tam jdu z polar plot visualisation screen kde kliknu na "apply filter" tak chci nacist ta data (a kdyz clovek pujde zpatky pres back, tak se to nacte ze state takze to nemusim resit v navigaci) => pridat anvigacni argument reload: Boolean a pak init metodu, ktera kdyz je relaod true, tak data nacte, jinak ne